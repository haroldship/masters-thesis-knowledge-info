...
This section describes the results of a single experiment with risk function having a single peak with with \glsentryname{spread} 1.0 and \glsentryname{factor} of 100 on a fixed, uniform population of 10,000.
These parameters are summarized in \cref{tab:params:unif_100_1.0_1h}.
\Cref{fig:cases_scatter:unif_100_1.0_1h} shows the distribution of the population in \subref{fig:cases_scatter:unif_100_1.0_1h:popdist},
...
\begin{table}[htbp]
    ...
    \caption[Parameters of single-peak risk of 100 on uniform population]
        {Parameters used in single-peak risk of \glsentryname{factor} 100 with \glsentryname{spread} of 1.0 on uniform population}
...
We also observe that the distributions are heavily skewed to the left,
indicating that for this simple setup,
the accuracy of the \gls{dkd} is much more likely to be below its mean.
Finally, we note that the estimates calculated with \gls{silverman} and \gls{cv} selected bandwidths and their corresponding mean values had similar performance in \gls{ise}.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption[\glsentryname{ise}: Single-peak of 100 on uniform population]{\glsentryname{ise} histogram for a single-peak intensity with \glsentryname{spread} of 1.0 and \glsentryname{factor} of 100 on a uniform population. Vertical lines indicate the estimated \glsentryname{mise} of the simulations.}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption[\glsentryname{iae}: Single-peak of 100 on uniform population]{\glsentryname{iae} histogram for a single-peak intensity with \glsentryname{spread} of 1.0 and \glsentryname{factor} of 100 on a uniform population. Vertical lines indicate the estimated \glsentryname{mise} of the simulations.}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \subcaption{Centroid peak heights}
    \end{subfigure}

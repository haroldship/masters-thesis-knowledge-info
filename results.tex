...
\Cref{sec:results:unif_100_1.0_1h} describes the results of a single experiment in detail.
Each of
\Cref{sec:results:number_of_incidents,sec:results:spread,sec:results:unifNpop_1h,sec:results:pop_spread,sec:results:p1.4_100_G}
describes a set of experiments, which, together,
examine the effect of changes to a single factor on the performance of the \gls{dkd}.
...
\Cref{sec:results:unifNpop_1h} compares the results obtained by
varying the sizes of the population and expected number of incidents in tandem,
while keeping the risk function fixed.
\Cref{sec:results:pop_spread} compares the results obtained by varying the spread of the population.
\Cref{sec:results:p1.4_100_G} we look at incident risk functions that have two peaks and vary the distance between them.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section
    {A single experiment with single peak risk function with \glsentryname{spread} 1.0 and \glsentryname{factor} of 100 on a fixed, uniform population of 10,000}
...
We simulate this phenomenon with a \gls{risk} function having a single peak with \glsentryname{spread} 1.0 and \glsentryname{factor} of 100.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{ll}
        ...
        \Glsentryname{factor} as percent of population & 1\% \\
...
\begin{table}[htbp]
    ...
    \caption[Mean error rates for uniform population single-peak risk]
        {Mean error rates for uniform population, single-peak risk with \glsentryname{spread} 1.0 of \glsentryname{factor} 100}
    \label{tab:errors:unif_100_1.0_1h}
\end{table}

\Cref{tab:errors:unif_100_1.0_1h} contains a summary of all of the measures of accuracy,
described in \Cref{sec:method:accuracy}, for this experiment.
...
described in \Cref{sec:theory:accuracy}.
...
(see \Cref{subsec:method:peak_bias}).
In this experiment,
...
While the mean error in the estimated peak height was improved $11\%$ by the centroid technique for the \gls{oracle} bandwidth,
the \gls{silverman} and \gls{cv} bandwidths showed improvements of $40\%$ and $70\%$ respectively.
Regarding the estimated peak drift, the centroid technique gave more accurate results $66\%$ of the time for the \gls{oracle} bandwidth,
while the \gls{silverman} and \gls{cv} bandwidths showed improvements of $83\%$ and $72\%$ of the time, respectively.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{ll}
        ...
        \Glsentryname{factor} as percent of population & 0.5\%, 1\%, 2\%, 5\%, 10\% \\
        Incident \glsentryname{spread} & 1.0 \\
        Incident center & (0,0) \\
        \bottomrule
    \end{tabular}
...
and answers our \Cref{thm:accuracy-affected:duration} (\cpageref{thm:accuracy-affected:duration}).
In particular,
our question is,
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
        \caption[A single realization of different \glsentryplural{factor}]
            {A single realization of different \glsentryplural{factor} from a single-peak risk on a uniform population.
            \scatterplotcaption.}
        \label{fig:one_sample:unif_NCases_1h}
\end{figure}

According to the theory reviewed in \Cref{sec:theory:accuracy},
...
This will help us answer \Cref{thm:accuracy-affected:rates} (\cpageref{thm:accuracy-affected:rates}).
...
We saw in \Cref{sec:theory:accuracy} that the bias, and hence the \gls{mse} of a kernel estimate at a point depends on $\lambda^{\prime \prime}$, the second derivitive of the true intensity.
...
and will provide us answers to our \Cref{thm:accuracy-affected:popsize} (\cpageref{thm:accuracy-affected:popsize}).
...
\begin{table}[htbp]
    ...
    \begin{tabular}{ll}
        ...
        Scaling factor & 100 \\
        Scaling factor as percent of population & 1\% \\
        Incident \gls{spread} & 1.0 \\
        Incident center & (0,0) \\
        \bottomrule
    \end{tabular}
...
fixed risk function in order to answer our \Cref{thm:accuracy-affected:popdist} (\cpageref{thm:accuracy-affected:popdist}).
...
There is no discernible difference between the \gls{silverman} and \gls{cv} selected bandwidths as far as \gls{mise} is concerned,
...
\begin{figure}[htbp]
    ...
    \caption[\glsentryname{miae} and \Glsentryname{supremum error} by population \glsentryname{spread}]
        {\glsentryname{miae} and \Glsentryname{supremum error} by population \glsentryname{spread}. \errorplotcaption}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{ll}
        ...
        Population center & uniform \\
        \Gls{factor} & 40, 60 \\
        \Glsentryname{factor} as percent of population & 0.4\%, 0.6\% \\
        Incident \gls{spread} & 1.0 \\
...
in order to provide more answers to our \Cref{thm:accuracy-affected:rates} (\cpageref{thm:accuracy-affected:rates}).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Peak bias and drift by peak distance
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.49\textwidth}
...
The \gls{peak drift} and \gls{centroid drift} increase when the distance between the two peaks increases (\Cref{fig:other_measures:p1.4_100_G:peak_drift,fig:other_measures:p1.4_100_G:centroid_drift}).
...
We also observe that \gls{peak bias} and \gls{centroid bias} decrease as the distance between the two peaks increases (\Cref{fig:other_measures:p1.4_100_G:peak_bias,fig:other_measures:p1.4_100_G:centroid_bias}).
...
\Cref{fig:method:overall_nmise_boxplot} shows the overall distribution between experiments of the \gls{nmise}.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \label{fig:method:overall_nmise_boxplot:unif}
    \end{subfigure}
    \begin{subfigure}[t]{0.45\textwidth}
        \includegraphics[width=\textwidth]{results/by_overall/normalized-mise-peakpop-boxplot}
        \subcaption{Peaked populations}
        \label{fig:method:overall_nmise_boxplot:peak}
    \end{subfigure}
    \caption[Overall distribution of \glsentryname{nmise}]
        {Overall distribution of \glsentryname{nmise}.
            The error scale in \autoref{fig:method:overall_nmise_boxplot:peak} is higher by a factor of 15.}
    \label{fig:method:overall_nmise_boxplot}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \subcaption{Uniform populations}
        \label{fig:method:overall_nmise_diff_boxplot:unif}
    \end{subfigure}
    \begin{subfigure}[t]{0.45\textwidth}
        \includegraphics[width=\textwidth]{results/by_overall/normalized-mise-diff-peakpop-boxplot}
        \subcaption{Peaked populations}
        \label{fig:method:overall_nmise_diff_boxplot:peak}
    \end{subfigure}
    \caption[Overall distribution of \glsentryname{nmise} difference from the \Glsentryname{oracle}]
        {Overall distribution of \glsentryname{nmise} difference from the \Glsentryname{oracle}.
            The error scale in \autoref{fig:method:overall_nmise_boxplot:peak} is higher by a factor of 20.}
    \label{fig:method:overall_nmise_diff_boxplot}
\end{figure}
...
\Cref{fig:method:overall_nmiae_boxplot} shows the distribution of \gls{nmiae} over the entire set of experiments.
We note the large difference in scale between the experiments with uniform populations shown in \subref{fig:method:overall_nmiae_boxplot:unif} and those with peaked populations in \subref{fig:method:overall_nmiae_boxplot:peak}.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \subcaption{Uniform populations}
        \label{fig:method:overall_nsup_boxplot:unif}
    \end{subfigure}
    \begin{subfigure}[t]{0.45\textwidth}
        \includegraphics[width=\textwidth]{results/by_overall/normalized-sup-error-peakpop-boxplot}
        \subcaption{Peaked populations}
        \label{fig:method:overall_nsup_boxplot:peak}
    \end{subfigure}
    \caption{Overall distribution of \glsentryname{normalized supremum error}}
    \label{fig:method:overall_nsup_boxplot}
\end{figure}

Our \gls{supremum error} measure intuitively describes the expected worst case error of the \gls{dkd} estimate.
\Cref{fig:method:overall_nsup_boxplot} shows the overall distribution of the \gls{supremum error} over our experiments.
The results for uniform and peaked populations are shown in \Cref{fig:method:overall_nsup_boxplot:unif,fig:method:overall_nsup_boxplot:peak} respectively.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \subcaption{Uniform populations}
        \label{fig:method:overall_peakdrift_boxplot:unif}
    \end{subfigure}
    \begin{subfigure}[t]{0.45\textwidth}
        \includegraphics[width=\textwidth]{results/by_overall/relative-peak-drift-peakpop-boxplot}
        \subcaption{Peaked populations}
        \label{fig:method:overall_peakdrift_boxplot:peak}
    \end{subfigure}
    \caption{Overall distribution of relative \glsentryname{peak drift}}
    \label{fig:method:overall_peakdrift_boxplot}
\end{figure}

The distributions we observed for \gls{peak bias} and \gls{peak drift} are shown in \Cref{fig:method:overall_peakbias_boxplot,fig:method:overall_peakdrift_boxplot}.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \subcaption{Uniform populations}
        \label{fig:method:overall_centroiddrift_boxplot:unif}
    \end{subfigure}
    \begin{subfigure}[t]{0.45\textwidth}
        \includegraphics[width=\textwidth]{results/by_overall/relative-centroid-drift-peakpop-boxplot}
        \subcaption{Peaked populations}
        \label{fig:method:overall_centroiddrift_boxplot:peak}
    \end{subfigure}
    \caption{Overall distribution of relative \glsentryname{centroid drift}}
    \label{fig:method:overall_centroiddrift_boxplot}
\end{figure}

The \gls{centroid bias} performed much better than the \gls{peak bias} for measuring the highest risk.
For the \gls{oracle} selected bandwidths,
the distribution was similar to the \gls{peak bias} for uniform populations
but much better for populations with peaked distributions.
However, for both \gls{silverman} and \gls{cv} bandwidths,
the \gls{centroid bias} was consistently lower in both groups.
...
This is indicated by the better performance of both the \gls{centroid bias}
and \gls{centroid drift} accuracy measures over the \gls{peak bias} and \gls{peak drift} respectively.

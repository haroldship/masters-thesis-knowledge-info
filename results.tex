...
In this experiment, the \gls{silverman} bandwidth produced a better \gls{peak bias} value than the other bandwidth selectors. 
The \gls{oracle} and \gls{cv} bandwidths resulted in negative biases,
which is the logical result since the general result of smoothing is to lower the high values and raise the low values.
The \gls{silverman} bandwidth, on the other hand, produced a positive value,
which indicates undersmoothing.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption[Peak errors: single peak on uniform]{Peak distance from truth and height histograms for uniform intensity on a uniform population, 100 cases}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption[Centroid errors: single peak on uniform]{Centroid peak distance from truth and height histograms for uniform intensity on a uniform population, 100 cases}
    \label{fig:centroids:unif_100_1.0_1h}
\end{figure}

The peak distances (\cref{fig:peaks:unif_100_1.0_1h:dist}) and peak height errors (\cref{fig:peaks:unif_100_1.0_1h:height}).

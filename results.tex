...
\caption[Effect of \glsentryname{factor} with fixed population]
    {Experimental parameter values varying \entryname{factor} for a fixed, uniform population of 10,000 and single peak with spread 1.0}
\label{tab:params:results:number_of_incidents}
\end{table}

In this section we examine and compare the results of five experiments where the \glspl{factor} of a single-peak risk function is varied.
...
The observed values for the power of $n$ as represented by $N_I$ are shown in \cref{tab:results:alpha_by_selector}.

...
In this chapter we discuss research related to our study.
\Cref{sec:theory} gives the theoretical background of the \gls{dkd},
including how incidents are modeled, how kernel estimation works,
and how the \gls{dkd} is used in epidemiological studies.
In \Cref{sec:literature} we discuss current research into kernel methods and applications of the \gls{dkd}.
...
the probability that the area \ds~ contains exactly one point is
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Kernel estimation of the intensity}
...
On the other hand,
non-parametric models generally tend to converge more slowly to the truth than parametric models,
...
\begin{equation}
    ...
    \gls{lambda_hat}_{1,h}(x) = \sumSone{K_1\left( \frac{x - S}{h} \right)} \text{.}%
\end{equation}
Here, $K_1(x)$ is a one-dimensional kernel function and the random variables $S \in N$ are the observed points.
...
\begin{defn}
    ...
    \end{equation}
\end{defn}
where $(S_1, S_2) \in N$ are the observed points,
$\xvec-(S_1, S_2)$ is ordinary vector subtraction,
...
As mentioned,
we estimate the \gls{risk} using the \gls{incidence rate} function.
In order to estimate the \gls{incidence rate} function,
we compute two kernel intensity estimates,
$\hat{\gls{lambda_I}}$ and $\hat{\gls{lambda_P}}$,
and take the quotient.
This gives us the following formal definition for the \gls{dkd} at any point $\xvec \in \R$:
\begin{defn}
    \label{defn:dkd-formula}
    The \textbf{\acrfull{dkd}} $\gls{lambda_hat} \xvec$
    is a technique for estimating the incidence rate
    using two \glspl{kernel intensity estimator}
    from observed \glspl{incident} and population \glspl{event} \gls{I} and \gls{P}.
    The formula for computing the \gls{dkd} is:
    \begin{equation}
        \label{eq:dkd-formula}
        \hat{\gls{lambda}}\xvec = \frac{\hat{\gls{lambda_I}}\xvec}{\hat{\gls{lambda_P}}\xvec}%
        \text{.}%
    \end{equation}
\end{defn}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Measuring the accuracy of the estimate}
...
\begin{defn}
    The \gls{mise} \textbf{optimal bandwidth} $\mathbf{\gls{h_opt}}$ is the value of $h$ that minimizes the \gls{mise} of the \gls{kde} for a specific probability density function $f$ and kernel function $K$.
\end{defn}
As noted above,
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Bandwidth selection from data}
...
\begin{equation}
    \sigma = \frac{\sigma_{x_1} + \sigma_{x_2}}{2} \text{.}
\end{equation}
...
For the \gls{kde},
the formula for the \gls{cv} error of bandwidth $h$ for \gls{lambda_hat} is
\begin{equation}
    \label{eq:cverror}
    \mbox{\Gls{cv}}%
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Inconsistency of the kernel intensity estimator}
...
Our $\gls{lambda}(x_1, x_2) \leq 1$ is a risk function,
and therefore a probability at every $(x_1, x_2) \in \gls{W}$.
These probabilities are independent of each other.
Therefore,
the number of \glspl{incident} is a sum of independent Bernoulli random variables,
although they have different distributions.
The distribution of the sum of independent Bernoulli random variables is a random variable
with a Poisson-Binomial distribution.
According to Le Cam's Theorem \citep{lecam1960},
this distribution approximates the Poisson Distribution for a large number of trials.
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Summary of research related to the DKD method}
...
\Citet{diggle2014statistical} extensively covers the spatial-statistical and spatio-temporal-statistical analysis of point patterns such as these.
...
\subsection{Kernel intensity estimation}
...
As a non-parametric method, \gls{kernel intensity estimation} has both advantages and disadvantages when compared to parametric techniques.
A general comparison of non-parametric and parametric approaches to analyzing \glspl{spp} was conducted by \citep{diggle2000comparison}.
In general,
parametric techniques perform better when their assumptions are known to hold,
but the non-parametric techniques were ``reasonably competitive''.
Furthermore, the non-parametric techniques were not sensitive to model mis-specification.

Despite the importance of the bandwidth selection method,
little research has been done on bandwidth selection for \gls{kernel intensity estimation} in the plane.
\Citet{fuentes2016consistent} describe a bootstrap-based technique for attributeless location data.
\Citet{borrajo2017bootstrapping} define two new bootstrap-based bandwidth selection methods
from combined location data in the plane and associated covariate data,
and analyze their behavior using monte carlo simulations.

\subsection{Application to epidemiology}

Current research in kernel estimation continues.
For example, we see kernel estimation techniques used in epidemiology,
where one of its uses is for mapping disease.
\Citet{bithell2000classification} compare different quantities and techniques of mapping disease.
...
In contrast to our study which computes the ratio of two \glspl{kernel intensity estimator},
they condition on sample size and compute two \glspl{kde} in order to compare the relative risk between two samples representing the case and control populations on a common study area.

\subsection{The double kernel}
...
\end{quotation}
\Citet{davarashvili2016application} compare zonal analysis to the \gls{dkd} in cancer incidence in the Haifa Bay area.
They found that the zonal models were unable to find any association between environmental factors and disease incidence,
whereas the \gls{dkd} was able to,
and so they recommend the \gls{dkd} for analysis of rare diseases.
Some other notable studies using the \gls{dkd} include \citet{svechkina2017new,svechkina2017spatial} which both use the \gls{dkd} to empirically
identify the locations of health hazards,
and \citet{rybnikova2018kernel} who used the \gls{dkd} to detect a halo pattern of breast cancer incidence in Connecticut
where there are high levels of artificial light.

...
\begin{enumerate}
    \item The true density $f$ has a second derivative $f^{\prime\prime}$ that is continous, square integrable, and ultimately monotone .
...
By our assumption, $\int{z^2K(z)\,\mathrm{d}z} < \infty$.
Therefore,
...
\begin{align}
\mbox{MSE}(\hat{f}(x;h)) & = (nh)^{-1} \int{K^2}f(x) + \sfrac{1}{4}h^4\left(\int{z^2K}\right)^2f^{\prime\prime}(x)^2 + o((nh)^{-1} + h^4) && \label{eq:AMSE}
\end{align}

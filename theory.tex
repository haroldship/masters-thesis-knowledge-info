...
\begin{enumerate}
    \item We create the incident \gls{intensity} function \gls{lambda_I}
        ...
        we get the expected number of \glspl{incident} generated by intensity $f$
        ...
        Therefore, if our desired expected number of incidents for a uniform population
        under \gls{intensity} \gls{lambda_I} is $N_I$,
        we must compute \gls{lambda_I} scale $f$ using the following formula:
        $$
            \gls{lambda_I}(p_1, p_2) = \frac{N_I}{N_P \times \mu_f} f(p_1, p_2)\text{.}
        $$

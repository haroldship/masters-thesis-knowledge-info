...
\begin{align}
    \mbox{MISE}(\hat{\lambda}) & = \E [\mbox{ISE}(\hat{\lambda})] \nonumber \\
                & = \E \iintW{ \left( \hat{\lambda}\xvec - \lambda\xvec \right)^2 } \label{eq:mise_base}
\end{align}

The integrand of \Cref{eq:mise_base} is non-negative.
Therefore, by Fubini's Theorem, we can change the order of integration and expectation.

\begin{align}
    \mbox{MISE}(\hat{\lambda}) & = \iintW{ \E \left( \hat{\lambda}\xvec - \lambda\xvec \right)^2 } \nonumber \\
                & = \iintW{ \E \left( \hat{\lambda}\xvec - \E \hat{\lambda}\xvec + \E \hat{\lambda}\xvec - \lambda\xvec \right)^2 } \nonumber \\
                & = \iintW{ \left( \hat{\lambda}\xvec - \E \hat{\lambda}\xvec \right)^2 } \nonumber \\
                    & \qquad + 2 \iintW{ \left( \hat{\lambda}\xvec - \E \hat{\lambda}\xvec \right) \left( \E \hat{\lambda}\xvec - \lambda\xvec \right) } \nonumber \\
                    & \qquad + \iintW{ \left( \E \hat{\lambda}\xvec - \lambda\xvec \right)^2 } \label{eq:mise_expanded}
\end{align}

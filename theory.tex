...
However, if we wish to compare the \gls{incidence rate} at different locations,
we need to look at these values locally.
If we knew these values exactly at a point $\xvec$,
then we could compute the \gls{incidence rate} point-wise.
This is the idea behind the \gls{dkd}.

\begin{defn}
    \label{defn:dkd-interim}
    The \textbf{\gls{dkd} (interim definition)} is a function that estimates the \gls{incidence rate} at a point by dividing the point-wise value of the number of new cases by the point-wise value of the population.
\end{defn}

We will give a more precise definition of the \gls{dkd} in \Cref{sec:theory:spatial_point_processes}.

...
In order to do this, we need to compute

\begin{align}
    \E \lmbdhat
        &= \E \left[ \sumS{\gls{K}\left(\frac{\xvec-(s_1, s_2)}{h^2}\right)} \right] \nonumber \\
        &= \frac{n^2}{h^2} \E \gls{K}\left(\frac{\xvec-(S_1, S_2)}{h^2}\right) 
            ~\text{where}~(S_1,S_2)~\text{is a random variable} \nonumber \\
\end{align}

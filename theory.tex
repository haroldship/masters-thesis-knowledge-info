...
This is similar to the two dimensional kernel density estimate,
used to estimate a probability density function.
Both are natural extensions to $\RS$ of the one dimensional kernel density estimate,
which has the formula

\begin{equation*}
    f(s) = \sumSone{K_1\left(\frac{x - s}{h}\right)} \text{.}%
\end{equation*}
...
The width of this area is controlled by the bandwidth \gls{h}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{img/kernel1d}
    \caption{One-dimensional \glsentryname{kernel} function. The dots on the $x1$ axis are the observed \glsentryplural{event}. The dashed lines are the kernel functions around these \glsentryplural{event} and the solid line is the estimate \gls{lambda_hat}\xvec.}

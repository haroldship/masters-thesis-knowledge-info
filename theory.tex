...
According to \Citet{diggle1988equivalence},
...
In \Cref{sec:derivations:biasvariancekernel}, we derive the \gls{amise} for the \gls{kde}.
...
we divide by $n$,
the number of observations.
This section is derived from \Citet{silverman1986density} and \Citet{wand1994kernel}.
In this section and in the related derivations in \Cref{ch:derivations},
...
See \Cref{sec:derivations:biasvariancekernel} for derivations for the \gls{kde}.
We wish to compute the bias of our \gls{kernel intensity estimator} \gls{lambda_hat},
where the bias is defined in \Cref{eq:estimator:bias}.
...
which are computed in \Cref{sec:derivations:biasvariancekernel}.
They are
\begin{align}

...
When the \textit{c1} and \textit{c2} parameters are given, the population is distributed according in a bivariate normal pattern.
...
In practice, we always use the same value for \textit{sigma1} and \textit{sigma2}.

Next, we generate the incident risk function, \(\gls{lambda}(x_1, x_2)\) parameter, by scaling it so that the expected number of incidents \gls{mu} is equal to the \textit{EN.i} parameter.

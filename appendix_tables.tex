% !TEX root = thesis.tex

%%
%%
%% results tables and figures appendix
%%
%%

%%
%% Section containing all uniform risk on uniform population
%%
\section{Uniform risk on a uniform population}
\label{sec:app:results_unif_unif}

%%
%% Tables and figures for uniform population of 10,000, uniform risk of factor 50
%%
\graphicspath{{./results/unif_50_unif/}}
\makeatletter
\def\input@path{{./results/unif_50_unif/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, uniform risk of \gls{factor} 50}
    \label{tab:mean_error_rates:unif_50_unif}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, uniform risk of \gls{factor} 50}
    \label{fig:distributions:unif_50_unif}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, uniform risk of factor 100
%%
\graphicspath{{./results/unif_100_unif/}}
\makeatletter
\def\input@path{{./results/unif_100_unif/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, uniform risk of \gls{factor} 100}
\label{tab:mean_error_rates:unif_100_unif}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, uniform risk of \gls{factor} 100}
    \label{fig:distributions:unif_100_unif}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, uniform risk of factor 200
%%
\graphicspath{{./results/unif_200_unif/}}
\makeatletter
\def\input@path{{./results/unif_200_unif/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, uniform risk of \gls{factor} 200}
\label{tab:mean_error_rates:unif_200_unif}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, uniform risk of \gls{factor} 200}
    \label{fig:distributions:unif_200_unif}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, uniform risk of factor 500
%%
\graphicspath{{./results/unif_500_unif/}}
\makeatletter
\def\input@path{{./results/unif_500_unif/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, uniform risk of \gls{factor} 500}
\label{tab:mean_error_rates:unif_500_unif}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, uniform risk of \gls{factor} 500}
    \label{fig:distributions:unif_500_unif}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, uniform risk of factor 1,000
%%
\graphicspath{{./results/unif_1000_unif/}}
\makeatletter
\def\input@path{{./results/unif_1000_unif/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, uniform risk of \gls{factor} 1,000}
\label{tab:mean_error_rates:unif_1000_unif}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, uniform risk of \gls{factor} 1,000}
    \label{fig:distributions:unif_1000_unif}    
\end{figure}

%%
%% Section containing single-peak risk with decay rate 0.7 on uniform population
%%
\section{Single-peak risk with decay rate 0.7 on a uniform population}
\label{sec:app:results_unif_0.7_1h}

%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 0.7, factor 50
%%
\graphicspath{{./results/unif_50_0.7_1h/}}
\makeatletter
\def\input@path{{./results/unif_50_0.7_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 0.7 and \gls{factor} 50}
    \label{tab:mean_error_rates:unif_50_0.7_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 0.7 and \gls{factor} 50}
    \label{fig:distributions:unif_50_0.7_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 0.7, factor 100
%%
\graphicspath{{./results/unif_100_0.7_1h/}}
\makeatletter
\def\input@path{{./results/unif_100_0.7_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 0.7 and \gls{factor} 100}
\label{tab:mean_error_rates:unif_100_0.7_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 0.7 and \gls{factor} 100}
    \label{fig:distributions:unif_100_0.7_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 0.7, factor 200
%%
\graphicspath{{./results/unif_200_0.7_1h/}}
\makeatletter
\def\input@path{{./results/unif_200_0.7_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 0.7 and \gls{factor} 200}
\label{tab:mean_error_rates:unif_200_0.7_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 0.7 and \gls{factor} 200}
    \label{fig:distributions:unif_200_0.7_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 0.7, factor 500
%%
\graphicspath{{./results/unif_500_0.7_1h/}}
\makeatletter
\def\input@path{{./results/unif_500_0.7_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 0.7 and \gls{factor} 500}
\label{tab:mean_error_rates:unif_500_0.7_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 0.7 and \gls{factor} 500}
    \label{fig:distributions:unif_500_0.7_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 0.7, factor 1,000
%%
\graphicspath{{./results/unif_1000_0.7_1h/}}
\makeatletter
\def\input@path{{./results/unif_1000_0.7_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 0.7 and \gls{factor} 1,000}
\label{tab:mean_error_rates:unif_1000_0.7_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 0.7 and \gls{factor} 1,000}
    \label{fig:distributions:unif_1000_0.7_1h}    
\end{figure}

%%
%% Section containing single-peak risk with decay rate 1.0 on uniform population
%%
\section{Single-peak risk with decay rate 1.0 on a uniform population}
\label{sec:app:results_unif_1.0_1h}

%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 1.0, factor 50
%%
\graphicspath{{./results/unif_50_1.0_1h/}}
\makeatletter
\def\input@path{{./results/unif_50_1.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 1.0 and \gls{factor} 50}
    \label{tab:mean_error_rates:unif_50_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 1.0 and \gls{factor} 50}
    \label{fig:distributions:unif_50_1.0_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 1.0, factor 100
%%
\graphicspath{{./results/unif_100_1.0_1h/}}
\makeatletter
\def\input@path{{./results/unif_100_1.0_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 1.0 and \gls{factor} 100}
\label{tab:mean_error_rates:unif_100_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 1.0 and \gls{factor} 100}
    \label{fig:distributions:unif_100_1.0_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 1.0, factor 200
%%
\graphicspath{{./results/unif_200_1.0_1h/}}
\makeatletter
\def\input@path{{./results/unif_200_1.0_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 1.0 and \gls{factor} 200}
\label{tab:mean_error_rates:unif_200_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 1.0 and \gls{factor} 200}
    \label{fig:distributions:unif_200_1.0_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 1.0, factor 500
%%
\graphicspath{{./results/unif_500_1.0_1h/}}
\makeatletter
\def\input@path{{./results/unif_500_1.0_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 1.0 and \gls{factor} 500}
\label{tab:mean_error_rates:unif_500_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 1.0 and \gls{factor} 500}
    \label{fig:distributions:unif_500_1.0_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 1.0, factor 1,000
%%
\graphicspath{{./results/unif_1000_1.0_1h/}}
\makeatletter
\def\input@path{{./results/unif_1000_1.0_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 1.0 and \gls{factor} 1,000}
\label{tab:mean_error_rates:unif_1000_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 1.0 and \gls{factor} 1,000}
    \label{fig:distributions:unif_1000_1.0_1h}    
\end{figure}

 
%%
%% Section containing single-peak risk with decay rate 1.4 on uniform population
%%
\section{Single-peak risk with decay rate 1.4 on a uniform population}
\label{sec:app:results_unif_1.4_1h}

%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 1.4, factor 50
%%
\graphicspath{{./results/unif_50_1.4_1h/}}
\makeatletter
\def\input@path{{./results/unif_50_1.4_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 1.4 and \gls{factor} 50}
    \label{tab:mean_error_rates:unif_50_1.4_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 1.4 and \gls{factor} 50}
    \label{fig:distributions:unif_50_1.4_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 1.4, factor 100
%%
\graphicspath{{./results/unif_100_1.4_1h/}}
\makeatletter
\def\input@path{{./results/unif_100_1.4_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 1.4 and \gls{factor} 100}
\label{tab:mean_error_rates:unif_100_1.4_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 1.4 and \gls{factor} 100}
    \label{fig:distributions:unif_100_1.4_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 1.4, factor 200
%%
\graphicspath{{./results/unif_200_1.4_1h/}}
\makeatletter
\def\input@path{{./results/unif_200_1.4_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 1.4 and \gls{factor} 200}
\label{tab:mean_error_rates:unif_200_1.4_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 1.4 and \gls{factor} 200}
    \label{fig:distributions:unif_200_1.4_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 1.4, factor 500
%%
\graphicspath{{./results/unif_500_1.4_1h/}}
\makeatletter
\def\input@path{{./results/unif_500_1.4_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 1.4 and \gls{factor} 500}
\label{tab:mean_error_rates:unif_500_1.4_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 1.4 and \gls{factor} 500}
    \label{fig:distributions:unif_500_1.4_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 1.4, factor 1,000
%%
\graphicspath{{./results/unif_1000_1.4_1h/}}
\makeatletter
\def\input@path{{./results/unif_1000_1.4_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 1.4 and \gls{factor} 1,000}
\label{tab:mean_error_rates:unif_1000_1.4_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 1.4 and \gls{factor} 1,000}
    \label{fig:distributions:unif_1000_1.4_1h}    
\end{figure}

 
 
%%
%% Section containing single-peak risk with decay rate 2.0 on uniform population
%%
\section{Single-peak risk with decay rate 2.0 on a uniform population}
\label{sec:app:results_unif_2.0_1h}

%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 2.0, factor 50
%%
\graphicspath{{./results/unif_50_2.0_1h/}}
\makeatletter
\def\input@path{{./results/unif_50_2.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 2.0 and \gls{factor} 50}
    \label{tab:mean_error_rates:unif_50_2.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 2.0 and \gls{factor} 50}
    \label{fig:distributions:unif_50_2.0_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 2.0, factor 100
%%
\graphicspath{{./results/unif_100_2.0_1h/}}
\makeatletter
\def\input@path{{./results/unif_100_2.0_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 2.0 and \gls{factor} 100}
\label{tab:mean_error_rates:unif_100_2.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 2.0 and \gls{factor} 100}
    \label{fig:distributions:unif_100_2.0_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 2.0, factor 200
%%
\graphicspath{{./results/unif_200_2.0_1h/}}
\makeatletter
\def\input@path{{./results/unif_200_2.0_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 2.0 and \gls{factor} 200}
\label{tab:mean_error_rates:unif_200_2.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 2.0 and \gls{factor} 200}
    \label{fig:distributions:unif_200_2.0_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 2.0, factor 500
%%
\graphicspath{{./results/unif_500_2.0_1h/}}
\makeatletter
\def\input@path{{./results/unif_500_2.0_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 2.0 and \gls{factor} 500}
\label{tab:mean_error_rates:unif_500_2.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 2.0 and \gls{factor} 500}
    \label{fig:distributions:unif_500_2.0_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 2.0, factor 1,000
%%
\graphicspath{{./results/unif_1000_2.0_1h/}}
\makeatletter
\def\input@path{{./results/unif_1000_2.0_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 2.0 and \gls{factor} 1,000}
\label{tab:mean_error_rates:unif_1000_2.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 2.0 and \gls{factor} 1,000}
    \label{fig:distributions:unif_1000_2.0_1h}    
\end{figure}


 
%%
%% Section containing single-peak risk with decay rate 2.8 on uniform population
%%
\section{Single-peak risk with decay rate 2.8 on a uniform population}
\label{sec:app:results_unif_2.8_1h}

%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 2.8, factor 50
%%
\graphicspath{{./results/unif_50_2.8_1h/}}
\makeatletter
\def\input@path{{./results/unif_50_2.8_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 2.8 and \gls{factor} 50}
    \label{tab:mean_error_rates:unif_50_2.8_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 2.8 and \gls{factor} 50}
    \label{fig:distributions:unif_50_2.8_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 2.8, factor 100
%%
\graphicspath{{./results/unif_100_2.8_1h/}}
\makeatletter
\def\input@path{{./results/unif_100_2.8_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 2.8 and \gls{factor} 100}
\label{tab:mean_error_rates:unif_100_2.8_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 2.8 and \gls{factor} 100}
    \label{fig:distributions:unif_100_2.8_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 2.8, factor 200
%%
\graphicspath{{./results/unif_200_2.8_1h/}}
\makeatletter
\def\input@path{{./results/unif_200_2.8_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 2.8 and \gls{factor} 200}
\label{tab:mean_error_rates:unif_200_2.8_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 2.8 and \gls{factor} 200}
    \label{fig:distributions:unif_200_2.8_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 2.8, factor 500
%%
\graphicspath{{./results/unif_500_2.8_1h/}}
\makeatletter
\def\input@path{{./results/unif_500_2.8_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 2.8 and \gls{factor} 500}
\label{tab:mean_error_rates:unif_500_2.8_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 2.8 and \gls{factor} 500}
    \label{fig:distributions:unif_500_2.8_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 2.8, factor 1,000
%%
\graphicspath{{./results/unif_1000_2.8_1h/}}
\makeatletter
\def\input@path{{./results/unif_1000_2.8_1h/}}
\makeatother

\begin{table}[H]
\input{error_table_fragment}
\caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 2.8 and \gls{factor} 1,000}
\label{tab:mean_error_rates:unif_1000_2.8_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 2.8 and \gls{factor} 1,000}
    \label{fig:distributions:unif_1000_2.8_1h}    
\end{figure}


%%
%% Section containing varied sample sizes on single-peak risk with decay rate 1.0 on uniform population
%%
\section{Varied sample sizes of single-peak risk on a uniform population}
\label{sec:app:results_unifXk_X_1.0_1h}

%%
%% Tables and figures for uniform population of 5,000, single-peak risk of decay rate 1.0, sample-size 50
%%
\graphicspath{{./results/unif5k_50_1.0_1h/}}
\makeatletter
\def\input@path{{./results/unif5k_50_1.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 5,000, single-peak risk of decay rate 1.0, sample-size 50}
    \label{tab:mean_error_rates:unif5k_50_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 5,000, single-peak risk of decay rate 1.0, sample-size 50}
    \label{fig:distributions:unif5k_50_1.0_1h}    
\end{figure}

%%
%% Tables and figures for uniform population of 10,000, single-peak risk of decay rate 1.0, sample-size 100
%%
\graphicspath{{./results/unif10k_100_1.0_1h/}}
\makeatletter
\def\input@path{{./results/unif10k_100_1.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, single-peak risk of decay rate 1.0, sample-size 100}
    \label{tab:mean_error_rates:unif10k_100_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, single-peak risk of decay rate 1.0, sample-size 100}
    \label{fig:distributions:unif10k_100_1.0_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 20,000, single-peak risk of decay rate 1.0, sample-size 200
%%
\graphicspath{{./results/unif20k_200_1.0_1h/}}
\makeatletter
\def\input@path{{./results/unif20k_200_1.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 20,000, single-peak risk of decay rate 1.0, sample-size 200}
    \label{tab:mean_error_rates:unif20k_200_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 20,000, single-peak risk of decay rate 1.0, sample-size 200}
    \label{fig:distributions:unif20k_200_1.0_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 50,000, single-peak risk of decay rate 1.0, sample-size 500
%%
\graphicspath{{./results/unif50k_500_1.0_1h/}}
\makeatletter
\def\input@path{{./results/unif50k_500_1.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 50,000, single-peak risk of decay rate 1.0, sample-size 500}
    \label{tab:mean_error_rates:unif50k_500_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 50,000, single-peak risk of decay rate 1.0, sample-size 500}
    \label{fig:distributions:unif50k_500_1.0_1h}    
\end{figure}


%%
%% Tables and figures for uniform population of 100,000, single-peak risk of decay rate 1.0, sample-size 1,000
%%
\graphicspath{{./results/unif100k_1000_1.0_1h/}}
\makeatletter
\def\input@path{{./results/unif100k_1000_1.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 100,000, single-peak risk of decay rate 1.0, sample-size 1,000}
    \label{tab:mean_error_rates:unif100k_1000_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 100,000, single-peak risk of decay rate 1.0, sample-size 1,000}
    \label{fig:distributions:unif100k_1000_1.0_1h}    
\end{figure}



%%
%% Section containing a single-peak risk with decay rate 1.0 on single-peak population of varying decay
%%
\section{Varied decay of single-peak population with single-peak risk}
\label{sec:app:results_pX_100_1.0_1h}

%%
%% Tables and figures for single-peak population of 10,000 with decay rate 0.7, single-peak risk of decay rate 1.0
%%
\graphicspath{{./results/p0.7_100_1.0_1h/}}
\makeatletter
\def\input@path{{./results/p0.7_100_1.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for single-peak population of 10,000 with decay rate 0.7, single-peak risk of decay rate 1.0}
    \label{tab:mean_error_rates:p0.7_100_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for single-peak population of 10,000 with decay rate 0.7, single-peak risk of decay rate 1.0}
    \label{fig:distributions:p0.7_100_1.0_1h}    
\end{figure}

%%
%% Tables and figures for single-peak population of 10,000 with decay rate 1.0, single-peak risk of decay rate 1.0
%%
\graphicspath{{./results/p1.0_100_1.0_1h/}}
\makeatletter
\def\input@path{{./results/p1.0_100_1.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for single-peak population of 10,000 with decay rate 1.0, single-peak risk of decay rate 1.0}
    \label{tab:mean_error_rates:p1.0_100_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for single-peak population of 10,000 with decay rate 1.0, single-peak risk of decay rate 1.0}
    \label{fig:distributions:p1.0_100_1.0_1h}    
\end{figure}

%%
%% Tables and figures for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0
%%
\graphicspath{{./results/p1.4_100_1.0_1h/}}
\makeatletter
\def\input@path{{./results/p1.4_100_1.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0}
    \label{tab:mean_error_rates:p1.4_100_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0}
    \label{fig:distributions:p1.4_100_1.0_1h}    
\end{figure}

%%
%% Tables and figures for single-peak population of 10,000 with decay rate 2.0, single-peak risk of decay rate 1.0
%%
\graphicspath{{./results/p2.0_100_1.0_1h/}}
\makeatletter
\def\input@path{{./results/p2.0_100_1.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for single-peak population of 10,000 with decay rate 2.0, single-peak risk of decay rate 1.0}
    \label{tab:mean_error_rates:p2.0_100_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for single-peak population of 10,000 with decay rate 2.0, single-peak risk of decay rate 1.0}
    \label{fig:distributions:p2.0_100_1.0_1h}    
\end{figure}

%%
%% Tables and figures for single-peak population of 10,000 with decay rate 2.8, single-peak risk of decay rate 1.0
%%
\graphicspath{{./results/p2.8_100_1.0_1h/}}
\makeatletter
\def\input@path{{./results/p2.8_100_1.0_1h/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for single-peak population of 10,000 with decay rate 2.8, single-peak risk of decay rate 1.0}
    \label{tab:mean_error_rates:p2.8_100_1.0_1h}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for single-peak population of 10,000 with decay rate 2.8, single-peak risk of decay rate 1.0}
    \label{fig:distributions:p2.8_100_1.0_1h}    
\end{figure}





%%
%% Section containing a double-peak risk with decay rates of 1.0 on uniform population of 10,000
%%
\section{Double-peak risk with decay rates of 1.0 on uniform population}
\label{sec:app:results_unif_100_1_2h_X}

%%
%% Tables and figures for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 1.0
%%
\graphicspath{{./results/unif_100_1_2h_1/}}
\makeatletter
\def\input@path{{./results/unif_100_1_2h_1/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 1.0}
    \label{tab:mean_error_rates:unif_100_1_2h_1}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 1.0}
    \label{fig:distributions:unif_100_1_2h_1}    
\end{figure}

%%
%% Tables and figures for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 2.0
%%
\graphicspath{{./results/unif_100_1_2h_2/}}
\makeatletter
\def\input@path{{./results/unif_100_1_2h_2/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 2.0}
    \label{tab:mean_error_rates:unif_100_1_2h_2}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 2.0}
    \label{fig:distributions:unif_100_1_2h_2}    
\end{figure}

%%
%% Tables and figures for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 3.0
%%
\graphicspath{{./results/unif_100_1_2h_3/}}
\makeatletter
\def\input@path{{./results/unif_100_1_2h_3/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 3.0}
    \label{tab:mean_error_rates:unif_100_1_2h_3}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 3.0}
    \label{fig:distributions:unif_100_1_2h_3}    
\end{figure}

%%
%% Tables and figures for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 4.0
%%
\graphicspath{{./results/unif_100_1_2h_4/}}
\makeatletter
\def\input@path{{./results/unif_100_1_2h_4/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 4.0}
    \label{tab:mean_error_rates:unif_100_1_2h_4}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 4.0}
    \label{fig:distributions:unif_100_1_2h_4}    
\end{figure}

%%
%% Section containing a single-peak risk with decay rates of 1.0 on single-peak population of 10,000
%%
\section{Single-peak risk with decay rate of 1.0 on single-peak population}
\label{sec:app:results_p1.4_100_1_1h_X}

%%
%% Tables and figures for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0, distance between peaks of 1.0
%%
\graphicspath{{./results/p1.4_100_1_1h_1s/}}
\makeatletter
\def\input@path{{./results/p1.4_100_1_1h_1s/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0, distance between peaks of 1.0}
    \label{tab:mean_error_rates:p1.4_100_1_1h_1s}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0, distance between peaks of 1.0}
    \label{fig:distributions:p1.4_100_1_1h_1s}    
\end{figure}

%%
%% Tables and figures for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0, distance between peaks of 2.0
%%
\graphicspath{{./results/p1.4_100_1_1h_2s/}}
\makeatletter
\def\input@path{{./results/p1.4_100_1_1h_2s/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0, distance between peaks of 2.0}
    \label{tab:mean_error_rates:p1.4_100_1_1h_2s}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0, distance between peaks of 2.0}
    \label{fig:distributions:p1.4_100_1_1h_2s}    
\end{figure}

%%
%% Tables and figures for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0, distance between peaks of 3.0
%%
\graphicspath{{./results/p1.4_100_1_1h_3s/}}
\makeatletter
\def\input@path{{./results/p1.4_100_1_1h_3s/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0, distance between peaks of 3.0}
    \label{tab:mean_error_rates:p1.4_100_1_1h_3s}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for single-peak population of 10,000 with decay rate 1.4, single-peak risk of decay rate 1.0, distance between peaks of 3.0}
    \label{fig:distributions:p1.4_100_1_1h_3s}    
\end{figure}

%%
%% Tables and figures for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 4.0
%%
\graphicspath{{./results/p1.4_100_1_1h_4s/}}
\makeatletter
\def\input@path{{./results/p1.4_100_1_1h_4s/}}
\makeatother

\begin{table}[H]
    \input{error_table_fragment}
    \caption[]{Error rates for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 4.0}
    \label{tab:mean_error_rates:p1.4_100_1_1h_4s}
\end{table}

\begin{figure}[H]
    \input{distribution_figures_fragment}
    \caption[]{Population distribution (a), true risk function (b), and sample population with incidents (c) for uniform population of 10,000, double-peak risk of decay rate 1.0, distance between peaks of 4.0}
    \label{fig:distributions:p1.4_100_1_1h_4s}    
\end{figure}

